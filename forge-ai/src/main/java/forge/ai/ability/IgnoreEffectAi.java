...
public class IgnoreEffectAi extends SpellAbilityAi {
    ...
    private boolean isSearchAbility(Player aiPlayer, SpellAbility sa, SpellAbility spell) {
        ...
        final boolean hasDefined = definedPlayer != null && (definedPlayer.equals("RememberedController")
                || definedPlayer.equals("TargetedController") || definedPlayer.equals("Player"));
        if (hasDefined) {
            final TargetRestrictions tgt = spell.getTargetRestrictions();
            if (tgt != null) {
                for (Card card : spell.getTargets().getTargetCards()) {
                    if (aiPlayer.equals(card.getController())) {
                        return true;
                    }
                }
                for (Player player : spell.getTargets().getTargetPlayers()) {
                    if (aiPlayer.equals(player)) {
                        return true;
                    }
                }
            }
     ...
     * 
     * @see
     * forge.card.abilityfactory.SpellAiLogic#canPlayAI(forge.game.player.Player,
     * java.util.Map, forge.card.spellability.SpellAbility)
     */
    @Override
    protected boolean canPlayAI(Player aiPlayer, SpellAbility sa) {
        if (aiPlayer.hasKeyword("CantSearchLibrary") && sa.getHostCard() != null
                && sa.getHostCard().getName().equals("Leonin Arbiter")) {
     ...
     * (non-Javadoc)
     * 
     * @see forge.card.abilityfactory.SpellAiLogic#chkAIDrawback(java.util.Map,
     * forge.card.spellability.SpellAbility, forge.game.player.Player)
     */
    @Override
